...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const searchButtonPhone = document.getElementById('search-button-phone');
  const searchOverlay = document.getElementById('search-overlay');
  const closeSearch = document.getElementById('close-search');
  

  // Show search field on clicking search icon
  searchButton.addEventListener('click', () => {
    searchOverlay.classList.remove('hidden');
  });
  // Show search field on clicking phone search icon
  searchButtonPhone.addEventListener('click', () => {
    searchOverlay.classList.remove('hidden');
  });

  // Hide search field on clicking close button or outside
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (!mainImage || cards.length === 0) {
    console.warn('Required elements not found. Check if IDs and classes are correct.');
    return;
  }
  ...
  cards.forEach((card) => {
    card.addEventListener('click', () => {
      try {
        ...
        cards.forEach((c) => {
          c.classList.remove('selected', 'bg-gray-50', 'border', 'border-gray-100', 'rounded-xl');
          ...
          if (h3) {
            h3.classList.remove('font-medium', 'text-gray-950', 'text-2xl', 'leading-8');
            h3.classList.add('text-2xl', 'text-gray-500', 'font-medium');
          }
          if (p) {
            p.classList.remove('pt-3', 'max-w-[300px]', 'font-normal', 'text-lg', 'leading-7', 'text-gray-700');
            p.classList.add('mt-3', 'text-gray-500', 'text-lg', 'font-normal');
          }
        });

        // Add selected state to clicked card
        card.classList.add('selected', 'bg-gray-50', 'border', 'border-gray-100', 'rounded-xl');
        const linkContainer = document.createElement('div');
        linkContainer.classList.add('link-container', 'flex', 'items-center', 'gap-0', 'mt-12');
        ...
        if (h3) {
          ...
          h3.classList.add('font-medium', 'text-gray-950', 'text-2xl', 'leading-8');
        }
        if (p) {
          p.classList.remove('mt-3', 'text-gray-500', 'text-lg', 'font-normal');
          p.classList.add('pt-3', 'max-w-[300px]', 'font-normal', 'text-lg', 'leading-7', 'text-gray-700');
        }
      } catch (error) {
